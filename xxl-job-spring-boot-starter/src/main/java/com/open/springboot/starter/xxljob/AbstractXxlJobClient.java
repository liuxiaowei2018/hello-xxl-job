...
public abstract class AbstractXxlJobClient implements XxlJobClient {
    ...
    public <T> T execute(BaseXxlJobRequest<T> request) throws XxlJobException {
        ...
        log.info("请求xxl-job api,url：{},req：{}", uri, JsonUtils.obj2json(request));
        ...
        if (UNCHECK_RESULT_API.contains(uri)) {
            return JsonUtils.json2pojo(resultBody, request.getResponseClass());
        }

        BaseXxlJobResponse<T> response = JsonUtils.json2pojo(resultBody, BaseXxlJobResponse.class);
        if (!response.isOk()) {
            log.error("xxl-job api[{}]执行失败,result：{}", uri, JsonUtils.obj2json(response));
    ...
    private String getCookie() {
        ...
        log.debug("获取xxl cookie,req：{}", JsonUtils.obj2json(request));
        HttpResponse response = HttpRequest.post(xxlJobProperties.getAdminAddresses() + request.getUrl())
                .form(request.toMap())
                .execute();
        if (!response.isOk()) {
            log.error("调用xxl获取cookie失败,response：{}", JsonUtils.obj2json(response.body()));
            throw new XxlJobException(ErrorConstant.INVOKE_XXL_GET_COOKIE_FAILED);
        }

        List<HttpCookie> cookies = response.getCookies();
        log.debug("获取xxl cookie成功,cookies：{}", JsonUtils.obj2json(cookies));
    ...
    public <T> T execute(BaseXxlJobApiRequest<T> request) {
        request.check();
        String uri = request.getUrl();
        log.info("请求xxl-job 开放api,url：{},req：{}", uri, JsonUtils.obj2json(request));
        String resultBody = HttpRequest.post(xxlJobProperties.getAdminAddresses() + uri)
                .body(JsonUtils.obj2json(request))
        ...
        BaseXxlJobResponse<T> response = JsonUtils.json2pojo(resultBody, BaseXxlJobResponse.class);
        if (!response.isOk()) {
            log.error("xxl-job 开放api[{}]执行失败,result：{}", uri, JsonUtils.obj2json(response));
            throw new XxlJobException(response.getMsg());
        }
        return response.getContent();
    }
